...
{“text”: “\U0001D54F”, “url”: “https://x.com/orbisapi?s=21”}
]]}
...
lines = [
    ...
    f"\U0001f4ca Total API Calls:   {total_calls}{_nl(d_calls)}",
    f"\U0001f517 APIs Listed:       {api_count}{d_apis}",
    "",
    f"\U0001f4b0 Subs Earned:   ${subs_earned} USDC{_nl(d_sub_earn)}",
    f"\u26a1 x402 Earned:   ${x402_earned} USDC{_nl(d_x402)}",
    f"\U0001f4ca Total Earned:  ${total_earned} USDC{_nl(d_total)}",
    "",
    "Top APIs:",
]
for a in apis[:5]:
...
if text == "/help":
    ...
    if is_admin:
        help_text += (
            ...
            "/allapi - Post all users to this chat\n"
...
if text == "/allapi":
    ...
    if not users:
        send_message("No users registered.", chat_id=chat_id)
        return
    send_message(f"\U0001f4e1 Posting {len(users)} users here...", chat_id=chat_id)
    for uid, user in users.items():
        try:
            data    = fetch_user_data(user.get("orbis_key", ""))
            caption = format_user_stats(uid, user, data, show_delta=True)
            send_with_media(
                caption, chat_id,
            ...
            save_prev_stats(uid, data.get("stats",{}), data.get("earnings",{}), data.get("x402",{}))
            time.sleep(1)
        except Exception as e:
            print(f"Error posting user {uid}: {e}")
    return
